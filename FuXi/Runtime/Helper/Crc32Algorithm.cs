	...
	internal class Crc32Algorithm : HashAlgorithm
	{
		...
		protected override byte[] HashFinal()
		{
			if (BitConverter.IsLittleEndian)
				return new[]
				{
					(byte) _currentCrc, (byte) (_currentCrc >> 8), (byte) (_currentCrc >> 16),
					(byte) (_currentCrc >> 24)
				};
			else
				return new[]
		...
		private static uint AppendInternal(uint initial, byte[] input, int offset, int length)
		{
			if (length > 0)
			{
				return _proxy.AppendSafe(initial, input, offset, length);
			}
			else
				return initial;
		}
		...
		private class SafeProxy
		{
			...
			public uint AppendSafe(uint crc, byte[] input, int offset, int length)
			{
